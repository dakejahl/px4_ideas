...
- The heading path should validate **baseline length + fix type** before publishing (oystub’s `vehicle_gnss_heading` bridge in #25516 does exactly this).
...
- Rover nav degrades when MB RTCM is late — and it is worse than a rate drop: in degraded MB mode the rover can fall to 1 Hz with navigation data **~900 ms stale** (Aviant measured via `time_utc_usec`, #25516; u-blox integration manual, GPSDrivers#109). That is outside the EKF delay horizon even if timestamp-compensated — **unfusable, not merely worse** — and the rover stays degraded for `CFG-NAVSPG-CONSTR_DGNSSTO` after corrections stop.
- **Coupled failure:** the MB dying is exactly what degrades the rover, so failover from MB lands on a fallback at its worst moment. Aviant hit toilet-bowl / position loss this way (vibration-sensitive MB going offline) and fixed it by **actively reconfiguring the rover out of rover mode on promotion** (disable RTCM input + soft position reset, ~1–2 s to recover; aviant-tech#104).
- **Tension:** rover often wins eph/fix_type; MB wins rate and latency stability.
- **Want:** role-aware policy, not pure eph. Product call: rate-first (MB) vs accuracy-first with rate veto. Aviant, running this topology commercially, converged on MB-primary + reconfigured-rover fallback (#25516) — a strong data point for rate-first.
...
- **Data latency as a health input**: GNSS time (`time_utc_usec`) vs system arrival time is a direct staleness measurement; data older than the EKF delay horizon is unfusable regardless of its quality flags (the degraded-rover case, §4 Case B). Demote on sustained latency, hard-floored at the EKF buffer depth.
- **Active role reconfiguration on promote (MB topologies)**: promoting a rover to position primary requires flipping it out of rover mode (disable RTCM input + soft position reset, ~1–2 s recovery — Aviant’s aviant-tech#104; slated as a follow-up to the `SensorGnssRelative` migration). `T_promote` must absorb the reconfig outage, and because MB failure *causes* rover degradation (coupled failure, §4 Case B), the selector should consider pre-emptively reconfiguring the rover as soon as MB health starts collapsing, not after demotion.
...
2. Implement the Layer-1 selector behind the existing `vehicle_gps_position` topic (all ~30 consumers unaffected); port the failover tests; make ekf2 replay + logging fully cover the selector and `sensor_gnss_relative` (the 2020 blending bugs were only found via replay, #14447 then broke it, and oystub’s port explicitly notes the missing replay plumbing).
...
- Direction: [#25516](https://github.com/PX4/PX4-Autopilot/pull/25516) (oystub: “hacky and semantically incorrect”; dev call: “blending mixes concepts that don’t really belong together”; dagar floats per-receiver EKF fusion; bengrocholsky: `EKF2_GPS_POS` array + rel-pos vector + 2-antenna partial / 3-antenna full attitude) → #27102. Also in #25516: measured **~900 ms staleness** on degraded MB rovers (“we avoid blending … rover-mode data cannot be trusted in these conditions”); rover-reconfiguration-on-promotion ([aviant-tech#104](https://github.com/aviant-tech/PX4-Autopilot/pull/104), Aviant 1.15 reference commit `a93400c8`); heading bridge validates baseline length + fix type; notes the ekf2-replay plumbing gap for `vehicle_gnss_relative`. Background: [GPSDrivers#109](https://github.com/PX4/PX4-GPSDrivers/pull/109) (MB rover nav-rate/delay, `CFG-NAVSPG-CONSTR_DGNSSTO`).
- Selection/metadata momentum: #26126 (prime = CAN node ID), #26634/#26660 (per-receiver offsets/delays), #26653 (lower default RTK timeout), GPSDrivers#201 (RTCM 1005 at 1 Hz), [DSDL#77](https://github.com/dronecan/DSDL/pull/77) (DroneCAN `gnss.quality` — future per-receiver health input for the selector), #27868 (telemetry follows prime). dagar architecture umbrellas: #15683, #22470.
